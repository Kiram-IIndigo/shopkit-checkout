        ...
        <div className="flex items-center gap-4 mb-10">
          {[
            ...
            <React.Fragment key={num}>
              <div
                className={`flex items-center gap-2 ${step >= parseInt(num) ? "text-white" : "text-gray-600"}`}
              >
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center font-mono text-xs border transition-all ${
                    step > parseInt(num)
                ...
                <span className="font-mono text-xs tracking-wider hidden sm:block">
                  {label}
                </span>
              </div>
              {i < 2 && (
                <div
                  className={`flex-1 h-px max-w-[60px] ${step > i + 1 ? "bg-green-500/50" : "bg-white/10"}`}
                />
              )}
            </React.Fragment>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            >
              <AddressForm
                data={billing}
                onChange={(field, val) => {
                  setBilling((p) => ({ ...p, [field]: val }));
                  setBillingErrors((p) => ({ ...p, [field]: "" }));
                }}
            ...
            >
              <form onSubmit={handleSubmit}>
                ...
                <div className="p-3 bg-accent/10 border border-accent/20 rounded-lg mb-5">
                  <p className="font-mono text-[10px] text-accent tracking-wider uppercase mb-1">
                    @Iindigo.graphic ·
                  </p>
          ...
          <div className="lg:col-span-1">
            <div className="card sticky top-24">
              <h2 className="font-display text-xl tracking-wider text-white mb-4">
                ORDER SUMMARY
              </h2>
...
function StepCard({ title, step, currentStep, onEdit, children }) {
  ...
  return (
    <div
      className={`card mb-4 transition-all duration-300 ${!isActive && !isComplete ? "opacity-40 pointer-events-none" : ""}`}
    >
      <div className="flex items-center justify-between mb-4">
        ...
        {isComplete && (
          <span className="w-6 h-6 rounded-full bg-green-500/20 border border-green-500/50 text-green-400 flex items-center justify-center text-xs ml-2">
            ✓
          </span>
        )}
      </div>
...
function AddressForm({
  ...
  const Field = ({ label, name, type = "text", placeholder, half }) => (
    <div className={half ? "" : "col-span-2 sm:col-span-1"}>
      ...
      />
      {errors[name] && (
        <p className="mt-1 text-red-400 text-xs font-mono">{errors[name]}</p>
      )}
    </div>
  );
