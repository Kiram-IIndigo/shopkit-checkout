          ...
          <div className="lg:col-span-2">
            ...
            <StepCard title="Payment" step={3} currentStep={step} onEdit={() => setStep(3)}>
              <form onSubmit={handleSubmit}>
                <div className="mb-5">
                  <label className="block font-mono text-xs tracking-widest uppercase text-gray-400 mb-2">Card Details</label>
                  <div className="input-field py-3.5">
                    <CardElement options={cardElementOptions} onChange={() => setCardError('')} />
                  </div>
                  {cardError && <p className="mt-2 text-red-400 text-xs font-mono">{cardError}</p>}
                </div>
            ...
            <div className="card sticky top-24">
              ...
              <div className="flex flex-col gap-3 mb-4 max-h-60 overflow-y-auto">
                {items.map((item) => (
                  <div key={item._id} className="flex gap-3 items-center">
                    ...
                    <span className="text-gray-300 text-xs font-mono">{(item.price * item.quantity).toFixed(2)} BAM</span>
                  </div>
                ))}
              </div>
              <div className="border-t border-white/8 pt-4 flex flex-col gap-2">
                <div className="flex justify-between font-mono text-xs text-gray-500">
                  <span>Subtotal</span><span>{subtotal.toFixed(2)} BAM</span>
                </div>
                <div className="flex justify-between font-mono text-xs text-gray-500">
                  <span>Shipping</span>
                ...
                <div className="flex justify-between font-mono text-lg text-white border-t border-white/8 pt-2 mt-1">
                  ...
                  <span className="text-accent font-display text-2xl">{total.toFixed(2)} BAM</span>
                </div>
              </div>
...
function AddressForm({ data, onChange, errors, showEmail = false, showPhone = false }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      {showPhone && (
        <Field label="Phone (optional)" name="phone" type="tel" placeholder="+387 61 000 000" data={data} onChange={onChange} errors={errors} />
      )}
      <div className="col-span-1 sm:col-span-2">
        <Field label="Street Address" name="address" placeholder="Ulica bb" data={data} onChange={onChange} errors={errors} />
      </div>
      <Field label="City" name="city" placeholder="Sarajevo" data={data} onChange={onChange} errors={errors} />
      <Field label="Postal Code" name="postalCode" placeholder="71000" data={data} onChange={onChange} errors={errors} />
      <div className="col-span-1 sm:col-span-2">
        <label className="block font-mono text-xs tracking-widest uppercase text-gray-400 mb-1.5">Country</label>
        <select
          value={data.country || 'BA'}
